...
def train(model, train_dataloader, epochs, optimizer, scheduler, checkpoint_path, validation_dataloader=None, device=torch.device('cuda')):
...
def main():
    ...
    # Check for multiple GPUs and use DataParallel if more than 1 GPU is available
    ...
    path, train_loss, validation_loss = train(model, train_dataloader, epochs, optimizer, scheduler, checkpoint_path, validation_dataloader=test_dataloader, device=device)
